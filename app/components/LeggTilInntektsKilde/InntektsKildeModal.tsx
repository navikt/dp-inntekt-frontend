...
import {Button, Label, Modal, Radio, RadioGroup, Select, TextField, VStack,} from "@navikt/ds-react";
  ...
  const { setInntektEndret, uklassifisertInntekt, setUklassifisertInntekt } = useInntekt();
    ...
    useEffect(() => {
        ...
        if (identifikator?.length === 9 && inntektskilde === "ORGANISASJON") {
            if (!ErVirksomhetsnummeretUnikt(identifikator)) {
                setEksisterendeVirksomhetVarsel(true)
                return;
            } else {
                hentVirksomhetsNavn();
            }
        }
        setEksisterendeVirksomhetVarsel(false)

    }, [identifikator]);

    function ErVirksomhetsnummeretUnikt(virksomhetsnummer: string): boolean {
        return !uklassifisertInntekt.virksomheter.some(virksomhet => virksomhet.virksomhetsnummer === virksomhetsnummer);
    }
                    ...
                    <form {...form.getFormProps()}>
                        <Modal.Body>
                            <VStack gap="4">
                                ...
                                <VStack gap="2">
                                    ...
                                    {eksisterendeVirksomhetVarsel && (
                                        <div className={styles.errorSummary}>
                                            Du kan ikke legge til en inntektskilde med et eksisterende organisasjonsnummer.
                                        </div>
                                    )}
                                </VStack>
                            </VStack>
                        ...
                        <Modal.Footer>
                            <Button
                                ...
                                disabled={eksisterendeVirksomhetVarsel}
                            >
                                Sett inn
                            </Button>
